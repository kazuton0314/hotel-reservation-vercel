...
- フォーム同期では、**同一人物（氏名＋連絡先）かつ同一チェックイン年月日**を重複候補として扱います。
- **取込済み判定は「行番号一致 かつ 同一人物・同一日」のときだけ**スキップします。シートをクリアして行番号が再利用されても、別人の新規回答は新規登録されます。
- `form_import_log` の行番号だけが残っていて中身が不一致の場合も再取込します。
